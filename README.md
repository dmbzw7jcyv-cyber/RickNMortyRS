...
### Late-replicating arm joint fix

The pose driver now discovers Motor6Ds as they arrive, waits for their Part0/Part1 links, and periodically checks unfinished rig replication. Previously it captured joints only once when the character model appeared; an empty/partial snapshot could leave punches inactive while crack effects continued. Activation no longer binds the old dead body, and a pose event arriving before the replacement character is retained until the character arrives or the existing timeout expires. Original joint poses and Animate state are restored, and discovery listeners disconnect on cleanup.

`tests/phoenix_pose.luau` exercises the production client Pose module with reflection-backed R6/R15 joints, including late joints, late Part1 links, contact/recoil, a missing character at event arrival, and cleanup. This reproduces the previous empty-joint failure offline; it does not replace Studio replication/visual testing. Force Breakout and verify right–left–right punches on both avatar types, including a second observing client and simulated network latency.
